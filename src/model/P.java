...
public class P extends Function {
    ...
    private double getA(int k) {
        return 2.0 * R * (2.0 * sin(k * PI / 4.0) - sin(k * PI / 2.0));
    }

    private double getB(int k) {
        return 2.0 * R * (2.0 * cos(k * PI / 4.0) + cos(k * PI / 2.0) - 1.0);
    }

    private double a0 = R * (0.125 + (PI - 1.0) / (2.0 * PI * R));
    ...
    public double getValue(double x) {
        ...
        for (int k = 1; k < n; k++) {
            double sum = 0.0;
            sum += 2.0 * R * getA(k) * sin(k * x / (2.0 * R));
            sum += 2.0 * R * getB(k) * cos(k * x / (2.0 * R));
